...
vector<string> generateCombinations(const vector<string>& wordset, int maxLength) {
...
string exhaustiveSearch() {
    for (int i = 0; i < pwlist.size(); i++) {
        if (check_password(pwlist[i], target_salt, target_pw_hash))
            return pwlist[i];
    }
...
main()
{
    ...
    pwlist = generateCombinations(wordset, wordset.size());
