...
string exhaustiveSearch(const string& charset, int maxLength) {
        ...
        for (const string& prefix : combinations) {
            for (char c : charset) {
                ...
                for (int i = 0; i < pwlist2.size(); i++) {
                    if ((newCombination + pwlist2[i]).length() <= MAX_LEN) {
                        string new_password = newCombination + pwlist2[i];
...
main()
{
    ...
    password = exhaustiveSearch(charset, charset.size());
